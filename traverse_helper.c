...
Error checkAssignment(Parse_Tree *assignment_statement)
{
  ...
  {
  case 1:
    to_assign = findType(assignment_statement->children[0]->children[0]);
    printf("to assign:\t%s\n", to_assign.name);
    printf("assignment:\t%s\n", assignment_statement->children[2]->name);
...
Error verifyExpression(Parse_Tree *expression, typeExpression left_type)
{
  ...
  printf("%s\n", expression->children[0]->name);
  if (strcmp(expression->children[0]->name, "arithmetic_expression") == 0)
  {
    if (strcmp(left_type.expression.typeName, "integer") == 0)
    {
      printf("----%s\t%d\n", left_type.expression.typeName, itr->num_of_children);
...
Error verifyTerm(Parse_Tree *term, typeExpression left_type)
{
  ...
  if (strcmp(left_type.expression.typeName, "integer") == 0)
  {
    char *trail_name, *trail_operator, *trail_type;
    while (itr->num_of_children == 3)
    {
      identifier_type = findType(itr->children[0]);
      trail_name = itr->children[0]->value;
      trail_operator = itr->children[1]->children[0]->value;
      trail_type = identifier_type.expression.typeName;
      identifier_type = findType(itr->children[0]);
      printf("itr .. %s\n", identifier_type.name);
      if ((strcmp(identifier_type.expression.typeName, "integer") != 0 && strcmp(identifier_type.expression.typeName, "constant") != 0) || strcmp(itr->children[1]->children[0]->value, "/") == 0)
      {
...
typeExpression findType(Parse_Tree *identifier)
{
  ...
  printf("not found\n");
  typeExpression dummy;
  strcpy(dummy.name, identifier->value);
  strcpy(dummy.expression.typeName, "constant");
  return dummy;
}